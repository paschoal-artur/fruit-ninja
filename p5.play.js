...
(function (root, factory) {
  ...
  else if (typeof exports === "object") factory(require("@code-dot-org/p5"));
   ...
   */
  p5.prototype.loadImageElement = function (
    path,
    successCallback,
    failureCallback
  ) {
   ...
   *
   */
  p5.prototype.imageElement = function (
    imgEl,
    sx,
    sy,
    sWidth,
    ...
    var vals = modeAdjust(dx, dy, dWidth, dHeight, this._renderer._imageMode);
    ...
    if (this._renderer._tint) {
      ...
      context.drawImage(imgEl, sx, sy, sWidth, sHeight, 0, 0, vals.w, vals.h);
  ...
  p5.prototype._mouseButtonIsPressed = function (buttonCode) {
    return (
      (this.mouseIsPressed && this.mouseButton === buttonCode) ||
  ...
  p5.prototype.mousePressedOver = function (sprite) {
    return (
      (this.mouseIsPressed || this.touchIsDown) && this.mouseIsOver(sprite)
    );
  };

  var styleEmpty = "rgba(0,0,0,0)";

  p5.Renderer2D.prototype.regularPolygon = function (
    x,
    y,
    sides,
    size,
  ...
  p5.prototype.regularPolygon = function (x, y, sides, size, rotation) {
    ...
    if (!this._renderer.isP3D) {
      this._validateParameters("regularPolygon", args, [
  ...
  */
  ...
  function Sprite(pInst, _x, _y, _w, _h) {
    ...
    */
    ...
    this._doSweptCollidersOverlap = function (target) {
      var displacement = this._getBroadPhaseCollider().collide(
        target._getBroadPhaseCollider()
      );
    ...
    this.mouseUpdate = function () {
      ...
      if (this.collider) {
        ...
        this.mouseIsOver = this.collider.overlap(
          new p5.PointCollider(mousePosition)
        );
    ...
    this.setCollider = function (
      ...
      if (
        ...
        throw new TypeError(
          'Usage: setCollider("' + type + '"[, offsetX, offsetY])'
        );
      } else if (
        _type === "circle" &&
        !(
        ...
        throw new TypeError(
          'Usage: setCollider("' + type + '"[, offsetX, offsetY[, radius]])'
        );
      } else if (
        _type === "aabb" &&
        !(
      ...
      if (_type === "point") {
        ...
        this.collider = p5.AxisAlignedBoundingBoxCollider.createFromSprite(
          this,
          offset,
          width,
          height
        ...
        this.collider = p5.OrientedBoundingBoxCollider.createFromSprite(
          this,
          offset,
          width,
          height,
    ...
    this.play = function () {
      ...
      if (
        ...
        this.animation.getFrame() === this.animation.images.length - 1
      ) {
    ...
    this.setAnimation = function (animationName) {
      ...
      var animation =
        pInst._predefinedSpriteAnimations &&
    ...
    this._collideWithOne = function (type, other, callback) {
      ...
      if (type === "bounce") {
        ...
        var thisInitialVelocity = p5.Vector.project(
          this.velocity,
          displacement
        );
        var otherInitialVelocity = p5.Vector.project(
          other.velocity,
          displacement
        );
    ...
    this._findDisplacement = function (target) {
      ...
      if (this._doSweptCollidersOverlap(target)) {
        ...
        var relativeVelocity = p5.Vector.sub(
          this.velocity,
          target.velocity
        ).mag();
        ...
        if (timestep < 1) {
          ...
          var thisDelta = p5.Vector.sub(
            thisOriginalPosition,
            this.previousPosition
          ).mult(timestep);
          var targetDelta = p5.Vector.sub(
  ...
  function Group() {
    ...
    function _groupCollide(type, target, callback) {
      ...
      for (var i = 0; i < this.size(); i++)
        didCollide =
          this.get(i)._collideWith(type, target, callback) || didCollide;
      return didCollide;
    }
    ...
    array.setRotateToDirectionEach = array.setPropertyEach.bind(
      array,
      "rotateToDirection"
    );
    array.setRotationEach = array.setPropertyEach.bind(array, "rotation");
    array.setRotationSpeedEach = array.setPropertyEach.bind(
      array,
      "rotationSpeed"
    );
    ...
    array.setSpeedAndDirectionEach = array.callMethodEach.bind(
      array,
  ...
  p5.prototype.createEdgeSprites = function () {
    ...
    this.leftEdge = this.createSprite(
      -edgeThickness / 2,
      height / 2,
      edgeThickness,
      height
    );
    this.rightEdge = this.createSprite(
      width + edgeThickness / 2,
      height / 2,
      edgeThickness,
      height
    );
    this.topEdge = this.createSprite(
      width / 2,
      -edgeThickness / 2,
      width,
      edgeThickness
    );
    this.bottomEdge = this.createSprite(
      width / 2,
  ...
  function Animation(pInst) {
    ...
    this.draw = function (x, y, r) {
      ...
      if (this.visible) {
        ...
        if (frame_info) {
          var missingX =
            (frame_info.sourceW || frame_info.width) - frame_info.width;
          var missingY =
            (frame_info.sourceH || frame_info.height) - frame_info.height;
  ...
  function SpriteSheet(pInst) {
    ...
    var shortArgs =
      spriteSheetArgs.length === 2 || spriteSheetArgs.length === 3;
    ...
    if (
      spriteSheetArgs[0] instanceof p5.Image ||
      spriteSheetArgs[0] instanceof Image
    ) {
    ...
    this.drawFrame = function (frame_name, x, y, width, height) {
      ...
      if (frameToDraw.spriteSourceSize && frameToDraw.sourceSize) {
        ...
        if (width) {
          x +=
            (frameToDraw.spriteSourceSize.x * dWidth) /
            frameToDraw.sourceSize.w;
        ...
        if (height) {
          y +=
            (frameToDraw.spriteSourceSize.y * dHeight) /
            frameToDraw.sourceSize.h;
      ...
      if (this.image instanceof Image) {
        pInst.imageElement(
          ...
          frameToDraw.frame.x,
          frameToDraw.frame.y,
          frameToDraw.frame.width,
          frameToDraw.frame.height,
          x,
          y,
          dWidth,
          dHeight
  ...
  Quadtree.prototype.getIndex = function (pRect) {
    if (!pRect.collider) return -1;
      ...
      var index = -1,
        ...
        topQuadrant =
          colliderBounds.top < horizontalMidpoint &&
          colliderBounds.bottom < horizontalMidpoint,
        //pRect can completely fit within the bottom quadrants
        bottomQuadrant = colliderBounds.top > horizontalMidpoint;

      //pRect can completely fit within the left quadrants
      if (
        colliderBounds.left < verticalMidpoint &&
        colliderBounds.right < verticalMidpoint
      ) {
        if (topQuadrant) {
          index = 1;
  ...
  p5.CollisionShape.prototype._onTransformChanged = function () {
    ...
    this._offset.set(0, 0).transform(this._localTransform);
  ...
  p5.CollisionShape.prototype.collide = function (other) {
    ...
    // separating axis.
    var candidateAxes = p5.CollisionShape._getCandidateAxesForShapes(
      displacee,
      displacer
    );
  ...
  p5.CircleCollider.createFromSprite = function (sprite, offset, radius) {
    ...
    var shape = new p5.CircleCollider(offset, customSize ? radius : 1);
  ...
  p5.CircleCollider.prototype.updateFromSprite = function (sprite) {
    if (this.getsDimensionsFromSprite) {
      if (sprite.animation) {
        this.radius =
          Math.max(sprite.animation.getWidth(), sprite.animation.getHeight()) /
          2;
      } else {
  ...
  p5.CircleCollider.prototype._getCandidateAxes = function (other) {
    ...
    if (
      other instanceof p5.OrientedBoundingBoxCollider ||
      other instanceof p5.AxisAlignedBoundingBoxCollider
    ) {
  ...
  p5.AxisAlignedBoundingBoxCollider.prototype = Object.create(
    p5.CollisionShape.prototype
  );
   ...
   * @return {p5.CircleCollider}
   */
  p5.AxisAlignedBoundingBoxCollider.createFromSprite = function (
    sprite,
    offset,
    width,
    height
   ...
   * @see p5.CollisionShape.prototype.getsDimensionsFromSprite
   */
  p5.AxisAlignedBoundingBoxCollider.prototype.updateFromSprite = function (
    sprite
  ) {
    if (this.getsDimensionsFromSprite) {
      if (sprite.animation) {
  ...
  p5.AxisAlignedBoundingBoxCollider.prototype._computeHalfDiagonals = function () {
    ...
    var composedTransform = p5.Transform2D.mult(
      this._parentTransform,
      this._localTransform
    );
  ...
  p5.AxisAlignedBoundingBoxCollider.prototype.draw = function (sketch) {
    ...
    sketch.rect(
      0,
      0,
      Math.abs(this._halfDiagonals[0].x) * 2,
      Math.abs(this._halfDiagonals[0].y) * 2
    );
    sketch.pop();
  };
   ...
   * @return {number}
   */
  p5.AxisAlignedBoundingBoxCollider.prototype._getRadiusOnAxis = function (
    axis
  ) {
  ...
  p5.OrientedBoundingBoxCollider.prototype = Object.create(
    p5.CollisionShape.prototype
  );
   ...
   * @return {p5.CircleCollider}
   */
  p5.OrientedBoundingBoxCollider.createFromSprite = function (
    sprite,
    offset,
    width,
    height,
   ...
   * @param {Sprite} sprite
   */
  p5.OrientedBoundingBoxCollider.prototype.updateSweptColliderFromSprite = function (
    sprite
  ) {
    var vMagnitude = sprite.velocity.mag();
    var vPerpendicular = new p5.Vector(sprite.velocity.y, -sprite.velocity.x);
    this._width =
      vMagnitude + 2 * sprite.collider._getRadiusOnAxis(sprite.velocity);
  ...
  p5.Transform2D.prototype.translate = function (arg0, arg1) {
    ...
    if (arg0 instanceof p5.Vector) {
      ...
      throw new TypeError(
        "Invalid arguments to Transform2D.translate: " + args
      );
    }
    return p5.Transform2D.mult([1, 0, x, 0, 1, y], this, this);
  };
  ...
  p5.Transform2D.prototype.scale = function (arg0, arg1) {
    ...
    if (arg0 instanceof p5.Vector) {
      ...
      throw new TypeError(
        "Invalid arguments to Transform2D.scale: " + arguments
      );
    }
    return p5.Transform2D.mult([sx, 0, 0, 0, sy, 0], this, this);
  };
  ...
  p5.Transform2D.prototype.rotate = function (radians) {
    ...
    if (typeof radians !== "number") {
      throw new TypeError(
        "Invalid arguments to Transform2D.rotate: " + arguments
      );
    }
    var sinR = Math.sin(radians);
    var cosR = Math.cos(radians);
    return p5.Transform2D.mult([cosR, -sinR, 0, sinR, cosR, 0], this, this);
  };
